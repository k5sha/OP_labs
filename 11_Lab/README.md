...
_SortedSet_ додає кілька специфічних методів для роботи з упорядкованими елементами:
...
4. Реалізації
    + Set:
      - Основні реалізації:
        1. HashSet (швидкий доступ, порядок не гарантується),
        2. LinkedHashSet (зберігає порядок додавання).
    + SortedSet:
        - Основна реалізація:
            1. TreeSet (реалізований на основі дерева, підтримує порядок).
            2. Для TreeSet порядок елементів забезпечується алгоритмом балансування (червоних-чорних дерев).

5. Використання
    + Set:
       - Використовується, коли унікальність елементів важлива, але порядок не має значення.
       - Няприклад, для зберігання унікальних ідентифікаторів або наборів без пріоритету.
...
У назві класу _TreeSet_ слово _Tree_ використовується тому, що ця структура даних реалізована на основі _двоїстого дерева пошуку_ (Binary Search Tree), а точніше — _червоних-чорних дерев_ (Red-Black Tree). Ця структура забезпечує ефективність операцій і впорядкованість елементів.

### 6. Якими критеріям мають відповідати об’єкти для того, щоб посилання на них можна було зберігати у TreeSet?

Щоб об’єкти могли зберігатися у TreeSet, вони повинні відповідати певним критеріям, які забезпечують коректну роботу з впорядкованою структурою:
1. Мати визначений порядок
2. Уникати дублювання
3. Забезпечити правильну реалізацію equals і hashCode (не обов’язково, але бажано)
4. Відсутність null (у більшості випадків)
5. Не бути взаємно непорівнюваними

### 7. Чому в TreeSet не слід зберігати посилання на мутабельні об’єкти?
Зберігати мутабельні об’єкти у TreeSet не рекомендується через те, як ця структура працює з впорядкуванням і унікальністю елементів: 
1. Проблема із впорядкуванням
2. Проблема з унікальністю
3. Проблеми з пошуком та видаленням
4. Баланс дерева не оновлюється
